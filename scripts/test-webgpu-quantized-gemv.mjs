...
const shaderMatch = source.match(
  /const\s+qwen2QuantizedMatrixMultiplySequenceShader\s*=\s*`([\s\S]*?)`;/,
);
if (!shaderMatch) {
  throw new Error("The scalar quantized matrix shader was not found");
}
...
(async () => {
  try {
    ...
    const inputSize = 32;
    const outputSize = 151936;
    const adapter = await navigator.gpu?.requestAdapter();
    if (!adapter) throw new Error("No WebGPU adapter is available");
    const device = await adapter.requestDevice();
    const workgroups = Math.ceil(outputSize / 64);
    if (workgroups > device.limits.maxComputeWorkgroupsPerDimension) {
      throw new Error(
        "Scalar GEMV dispatch " + workgroups + " exceeds device limit " +
          device.limits.maxComputeWorkgroupsPerDimension,
      );
    }
    const input = Float32Array.from(
      { length: inputSize },
      (_, index) => (index - 15.5) / 16,
    );

    async function runCase(quantType) {
      const rowByteLength = quantType === 4 ? 18 : 34;
      ...
      for (let row = 0; row < outputSize; row += 1) {
        ...
        if (quantType === 4) {
          for (let index = 0; index < 16; index += 1) {
            ...
            weight[rowOffset + 2 + index] = low | (high << 4);
          ...
          for (let index = 0; index < inputSize; index += 1) {
            ...
            weight[rowOffset + 2 + index] = quantized & 0xff;
      ...
      return {
        quantType,
        maximumError,
        boundaryRows: Array.from(actual.slice(65534, 65537)),
        finalRow: actual[outputSize - 1],
      };
    }
...
if (
  result.status !== 0 ||
  (!output.includes('"ok":true') && !output.includes("&quot;ok&quot;:true"))
) {

...
export async function layerNormGpu(
...
fn main() {
  ...
  for (var index = 0u; index < featureSize; index = index + 1u) {
    mean = mean + input[inputOffset + index];
  }
  mean = mean / f32(featureSize);

  var variance = 0.0;
  for (var index = 0u; index < featureSize; index = index + 1u) {
    let centered = input[inputOffset + index] - mean;
  ...
  for (var index = 0u; index < featureSize; index = index + 1u) {
    let normalized = (input[inputOffset + index] - mean) * scale;
